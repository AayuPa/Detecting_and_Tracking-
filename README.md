...
- **Team assignment** — for each player crop, the top half is masked in HSV to remove
  ...
  Assignment is cached per `track_id`, so clustering runs once per new player rather
  than once per frame.
- **Rendering** — corner-bracket boxes in team colour, yellow circle on the ball,
  green brackets on referees. Output dimensions and frame rate are inherited from the
  source video.
- **Track caching** — detections can be pickled to a stub, so the annotation code can
  be iterated on without re-running inference.
...
- Pitch homography / field-coordinate mapping
...
`notebooks/train_detector.ipynb` reproduces this. Note it needs a GPU runtime —
at 960px on CPU a single epoch runs into the hours.
...
Weights are committed at `models/best.pt` (39MB) — no extra download step.
...
| `--stub` | `None` | Pickle path for cached tracks — written if absent, read if present |
...
- **Domain gap.** The detector was fine-tuned on the Roboflow set, which is broadcast
  ...
  expect degradation and validate on target footage before quoting numbers.
...
- **Team assignment is fitted once**, on the first frame containing two or more
  ...
  containing players from only one team, will skew the clusters. It has no concept of
  goalkeepers or referees wearing a third colour.
